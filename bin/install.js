...
This directory contains vertical slicing analysis documents generated by Bokata Slicer CC.
...
**Project Analysis (multiple features):**
\`\`\`
...
\`\`\`
/bokata-feature Feature: User Browses Catalog
\`\`\`

### Optional Strategic Planning

After running core analysis, use these to generate implementation strategies:

**Implementation Paths (3-5 options with timelines):**
\`\`\`
/bokata-iterations-paths
\`\`\`

**Selection Matrix (complete catalog with dependencies):**
\`\`\`
/bokata-matrix
\`\`\`
...
**Core Analysis contains:**
- Executive Summary
- Complete feature/step breakdown
- Walking Skeleton (recommended minimum implementation)
- Dependency Analysis (what each increment requires/provides)

**Implementation Paths contain:**
- 3-5 strategic options with estimated timelines
- Decision guide for path selection
- Next steps

**Selection Matrix contains:**
...
All features must follow the **Actor + Action** format:
- ✅ Good: "User Browses Catalog"
- ✅ Good: "Admin Manages Users"
- ❌ Bad: "Catalog browsing"
...
`;
  ...
  if (installedCommands.length) installedCommands.forEach(c => log(`  /${c}`, 'green'));
    ...
    log('  /bokata - Project analysis (multiple features)', 'green');
    log('  /bokata-feature - Single feature analysis', 'green');
    log('  /bokata-iterations-paths - Implementation strategies (optional)', 'green');
    log('  /bokata-matrix - Selection matrix with dependencies (optional)', 'green');
  }
  ...
  log('\nQuick Start Examples:', 'bright');
  log('  /bokata Project: E-commerce with catalog, cart, checkout', 'blue');
  log('  /bokata-feature Feature: User Browses Catalog\n', 'blue');
  log('  Then optionally run:', 'blue');
  log('  /bokata-iterations-paths  # Get implementation strategies', 'blue');
  log('  /bokata-matrix           # Get complete dependency reference\n', 'blue');

  log('Output location:', 'bright');
  log('  ./docs/slicing-analysis/\n', 'blue');

  log('Documentation:', 'bright');
  log('  ./docs/slicing-analysis/README.md', 'blue');
  log('  https://github.com/abrahamvallez/bokata-slicer-cc\n', 'blue');
}
...
if (require.main === module) {
  try {
    ...
    log(`\nPlease report this issue at: https://github.com/abrahamvallez/bokata-slicer-cc/issues\n`, 'yellow');
    process.exit(1);
  }
}
